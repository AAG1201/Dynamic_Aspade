...
def spade_segmentation(clipped_signal, resampled_data, Ls, win_len, win_shift, ps_maxit, ps_epsilon, ps_r, ps_s, F_red, masks, dynamic):
  ...
  for n in tqdm(range(N), desc="Processing", unit="iteration"):
    ...
    # perform SPADE
    if dynamic:
      data_rec_block = dynamic_aspade(data_block, masks_seg, ps_Ls, ps_maxit, ps_epsilon, ps_r, ps_s,F_red)
    else:
      data_rec_block = aspade(data_block, masks_seg, ps_Ls, ps_maxit, ps_epsilon, ps_r, ps_s,F_red)
